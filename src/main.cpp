...
void Visit(const koopa_raw_function_t &func) 
{
    ...
    int spForEachInst = 0, spForRa = 0, spForFuncParams = 0;
    int maxLenForArgs = 0;
    ...
    func2sp[func] = spForAll;
    func2ra[func] = spForRa;
...
void Visit(const koopa_raw_store_t &rawStore) 
{
  if (rawStore.value->kind.tag == KOOPA_RVT_FUNC_ARG_REF)
  {
    ...
    if(myIndex < 8)
      writeTo(rawStore.dest, "a" + to_string(myIndex));
    else
    {
      int spNumForCalleeArgs = func2sp[curFunc] + (myIndex - 8) * 4;
